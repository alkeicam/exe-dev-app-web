            ...
            fileReader.onloadend = () => {
                ...
                for(let i=0; i<lines.length; i++){
                    ...
                    try{
                        ...
                        let role = columns[3].trim();
                        // let role = columns[3].toLowerCase();
                        // role = role.charAt(0).toUpperCase() + role.slice(1);
                        participant.name = `${columns[0].trim()} ${columns[1].trim()}`;
                        participant.email = columns[2].trim()
                        participant.role = role
                                                                        
                        if(!["Developer", "FrontendDeveloper", "BackendDeveloper", "MobileDeveloper", "FullstackDeveloper", "TechLead", "Manager", "Director"].includes(participant.role)) throw new Error(`Invalid entry at line ${lineNo} - role must be one of Developer, FrontendDeveloper, BackendDeveloper, MobileDeveloper, FullstackDeveloper, TechLead, Manager, Director`)
                        if(["MANAGER", "DIRECTOR"].includes(role.toUpperCase())){
                            ...
                            if(columns.length == 5 && columns[4]) 
                                participant.password = columns[4].trim()
                            else
                ...
                if(errors.length == 0) this.model.modals.m3.valid = true;
